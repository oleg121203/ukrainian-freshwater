...
export function useAudio() {
   ...
   */
  const playBubbleSound = useCallback((config: AudioConfig = {}) => {
    ...
    try {
      ...
      oscillator.frequency.exponentialRampToValueAtTime(200 * playbackRate, audioContext.currentTime + 0.3)
   ...
   */
  const playRippleSound = useCallback((config: AudioConfig = {}) => {
    const audioContext = getAudioContext()
    if (!audioContext) return
   ...
   */
  const playClickSound = useCallback((config: AudioConfig = {}) => {
    const audioContext = getAudioContext()
    if (!audioContext) return
    ...
    try {
      ...
      oscillator.frequency.exponentialRampToValueAtTime(800 * playbackRate, audioContext.currentTime + 0.1)
   ...
   */
  const playSwooshSound = useCallback((config: AudioConfig = {}) => {
    const audioContext = getAudioContext()
    if (!audioContext) return
   ...
   */
  const playAmbientSound = useCallback((config: AudioConfig = {}) => {
    const audioContext = getAudioContext()
  ...
  return useMemo(() => ({
    ...
    playAmbientSound,
    resumeAudioContext
  }), [playBubbleSound, playRippleSound, playClickSound, playSwooshSound, playAmbientSound, resumeAudioContext])
}